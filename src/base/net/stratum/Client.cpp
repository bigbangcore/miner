...
xmrig::Client::Client(int id, const char *agent, IClientListener *listener)
    : BaseClient(id, listener), m_agent(agent) {
...
int64_t xmrig::Client::submit(const JobResult &result) {
  ...
  Buffer::toHex(reinterpret_cast<const char *>(&result.time), 4, time);
  time[8] = '\0';
#endif
  ...
  params.AddMember("time", StringRef(time), allocator);
  params.AddMember("result", StringRef(data), allocator);

  if (has<EXT_ALGO>() && result.algorithm.isValid()) {
    params.AddMember("algo", StringRef(result.algorithm.shortName()),
                     allocator);
  }
...
#ifdef XMRIG_PROXY_PROJECT
  m_results[m_sequence] =
      SubmitResult(m_sequence, result.diff, result.actualDiff(), result.id);
#else
  m_results[m_sequence] =
      SubmitResult(m_sequence, result.diff, result.actualDiff());
#endif
...
bool xmrig::Client::parseJob(const rapidjson::Value &params, int *code) {
  ...
  if (job.algorithm().family() == Algorithm::RANDOM_X &&
    ...
    if (!isQuiet()) {
      LOG_ERR("[%s] failed to parse field \"seed_hash\" required by RandomX",
              url(), algo);
    }
...
bool xmrig::Client::verifyAlgorithm(const Algorithm &algorithm,
                                    const char *algo) const {
  if (!algorithm.isValid()) {
    if (!isQuiet()) {
      LOG_ERR("[%s] Unknown/unsupported algorithm \"%s\" detected, reconnect",
              url(), algo);
    }
  ...
  if (!ok && !isQuiet()) {
    LOG_ERR("[%s] Incompatible/disabled algorithm \"%s\" detected, reconnect",
            url(), algorithm.shortName());
  }
...
int xmrig::Client::resolve(const String &host) {
  ...
  if (!m_dns->resolve(host)) {
    if (!isQuiet()) {
      LOG_ERR("[%s:%u] getaddrinfo error: \"%s\"", host.data(), m_pool.port(),
              uv_strerror(m_dns->status()));
    }
...
int64_t xmrig::Client::send(const rapidjson::Document &doc) {
  ...
  if (size > (sizeof(m_sendBuf) - 2)) {
    LOG_ERR("[%s] send failed: \"send buffer overflow: %zu > %zu\"", url(),
            size, (sizeof(m_sendBuf) - 2));
    close();
    return -1;
  }
...
int64_t xmrig::Client::send(size_t size) {
  LOG_DEBUG("[%s] send (%d bytes): \"%.*s\"", url(), size,
            static_cast<int>(size) - 1, m_sendBuf);
...
void xmrig::Client::parse(char *line, size_t len) {
  ...
  if (doc.ParseInsitu(line).HasParseError()) {
    if (!isQuiet()) {
      LOG_ERR("[%s] JSON decode failed: \"%s\"", url(),
...
void xmrig::Client::parseNotification(const char *method,
                                      const rapidjson::Value &params,
                                      const rapidjson::Value &error) {
  if (error.IsObject()) {
    if (!isQuiet()) {
      LOG_ERR("[%s] error: \"%s\", code: %d", url(),
              error["message"].GetString(), error["code"].GetInt());
    }
    return;
  }
...
void xmrig::Client::parseResponse(int64_t id, const rapidjson::Value &result,
                                  const rapidjson::Value &error) {
  if (error.IsObject()) {
    const char *message = error["message"].GetString();

    if (!handleSubmitResponse(id, message) && !isQuiet()) {
      LOG_ERR("[%s] error: " RED_BOLD("\"%s\"") RED_S ", code: %d", url(),
              message, error["code"].GetInt());
    }
...
void xmrig::Client::ping() {
  send(snprintf(m_sendBuf, sizeof(m_sendBuf),
                "{\"id\":%" PRId64
                ",\"jsonrpc\":\"2.0\",\"method\":\"keepalived\",\"params\":{"
                "\"id\":\"%s\"}}\n",
...
void xmrig::Client::startTimeout() {
  ...
  if (has<EXT_KEEPALIVE>()) {
    const uint64_t ms = static_cast<uint64_t>(m_pool.keepAlive() > 0
                                                  ? m_pool.keepAlive()
...
void xmrig::Client::onConnect(uv_connect_t *req, int status) {
  ...
  if (status < 0) {
    ...
    if (client->state() != ConnectingState) {
      LOG_ERR("[%s] connect error: \"invalid state: %d\"", client->url(),
              client->state());
...
void xmrig::Client::onRead(uv_stream_t *stream, ssize_t nread,
                           const uv_buf_t *) {
