 ...
 * <https://github.com/vtnerd> Copyright 2018      SChernykh
...
xmrig::Job::Job() : m_blob(), m_seedHash() {}
...
bool xmrig::Job::isEqual(const Job &other) const {
  return m_id == other.m_id && m_clientId == other.m_clientId &&
         memcmp(m_blob, other.m_blob, sizeof(m_blob)) == 0;
}
...
bool xmrig::Job::setTarget(const char *target) {
  ...
  if (len <= 8) {
    ...
    if (!Buffer::fromHex(str, 8, reinterpret_cast<uint8_t *>(&tmp)) ||
        tmp == 0) {
      return false;
    }

    m_target =
        0xFFFFFFFFFFFFFFFFULL / (0xFFFFFFFFULL / static_cast<uint64_t>(tmp));
  } else if (len <= 16) {
    m_target = 0;
    char str[16];
    memcpy(str, target, len);
    ...
    if (!Buffer::fromHex(str, 16, reinterpret_cast<uint8_t *>(&m_target)) ||
        m_target == 0) {
      return false;
    }
    // printf("m_target: %llx\n", m_target);
    // std::cout << "m_target:" << std::hex << m_target << std::endl;
  } else {
    return false;
  }
...
void xmrig::Job::setDiff(uint64_t diff) {
  m_diff = diff;
  // std::cout << "m_diff:" << std::hex << m_diff << std::endl;
