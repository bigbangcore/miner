...
xmrig::Network::Network(Controller *controller)
    : m_controller(controller), m_donate(nullptr), m_timer(nullptr) {
...
void xmrig::Network::connect() { m_strategy->connect(); }
...
void xmrig::Network::onActive(IStrategy *strategy, IClient *client) {
  ...
  LOG_INFO(WHITE_BOLD("use %s ") CYAN_BOLD("%s:%d ")
               GREEN_BOLD("%s") " " BLACK_BOLD("%s"),
           client->mode(), client->pool().host().data(), client->pool().port(),
           tlsVersion ? tlsVersion : "", client->ip().data());
...
void xmrig::Network::onJob(IStrategy *strategy, IClient *client,
                           const Job &job) {
  if (m_donate && m_donate->isActive() && m_donate != strategy) {
    return;
  }

  //std::cout<<"job.time:"<<*job.time()<<std::endl;
...
void xmrig::Network::onLogin(IStrategy *, IClient *client,
...
void xmrig::Network::onResultAccepted(IStrategy *, IClient *,
  ...
  if (error) {
    LOG_INFO(
        RED_BOLD("rejected") " (%" PRId64 "/%" PRId64 ") diff " WHITE_BOLD(
            "%" PRIu64) " " RED("\"%s\"") " " BLACK_BOLD("(%" PRIu64 " ms)"),
        m_state.accepted, m_state.rejected, result.diff, error, result.elapsed);
  } else {

    LOG_INFO(
        GREEN_BOLD("accepted") " (%" PRId64 "/%" PRId64 ") diff " WHITE_BOLD(
            "%" PRIu64) " " BLACK_BOLD("(%" PRIu64 " ms)"),
        m_state.accepted, m_state.rejected, result.diff, result.elapsed);
  }
}

void xmrig::Network::onVerifyAlgorithm(IStrategy *, const IClient *,
                                       const Algorithm &algorithm, bool *ok) {
...
void xmrig::Network::setJob(IClient *client, const Job &job, bool donate) {
  if (job.height()) {
    LOG_INFO(
        MAGENTA_BOLD("new job") " from " WHITE_BOLD("%s:%d") " diff"
                                                             " " WHITE_BOLD("%" PRIu64) " algo " WHITE_BOLD(
                                                                 "%s") " height"
                                                                       " " WHITE_BOLD("%" PRIu64),
        client->pool().host().data(), client->pool().port(), job.diff(),
        job.algorithm().shortName(), job.height());
  } else {
    LOG_INFO(
        MAGENTA_BOLD("new job") " from " WHITE_BOLD(
            "%s:%d") " diff " WHITE_BOLD("%" PRIu64) " algo " WHITE_BOLD("%s"),
        client->pool().host().data(), client->pool().port(), job.diff(),
...
#ifdef XMRIG_FEATURE_API
void xmrig::Network::getConnection(rapidjson::Value &reply,
                                   rapidjson::Document &doc,
                                   int version) const {
  using namespace rapidjson;
  auto &allocator = doc.GetAllocator();
  ...
  reply.AddMember(
      "algo", StringRef(m_strategy->client()->job().algorithm().shortName()),
      allocator);
  ...
  connection.AddMember("tls-fingerprint", m_state.fingerprint().toJSON(),
                       allocator);
...
void xmrig::Network::getResults(rapidjson::Value &reply,
                                rapidjson::Document &doc, int version) const {
  using namespace rapidjson;
  auto &allocator = doc.GetAllocator();
  ...
  results.AddMember("shares_total", m_state.accepted + m_state.rejected,
                    allocator);
