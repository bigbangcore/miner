...
public:
  ...
  inline JobResult(const Job &job, uint32_t nonce, uint32_t time,
                   const uint8_t *result)
      : algorithm(job.algorithm()), clientId(job.clientId()), jobId(job.id()),
        nonce(nonce), diff(job.diff()), index(job.index()), time(time) {
