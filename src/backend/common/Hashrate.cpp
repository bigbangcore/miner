...
xmrig::Hashrate::Hashrate(size_t threads) : m_highest(0.0), m_threads(threads) {
...
double xmrig::Hashrate::calc(size_t threadId, size_t ms) const {
  ...
  for (size_t i = 1; i < kBucketSize; i++) {
    ...
    if (xmrig::Chrono::highResolutionMSecs() - m_timestamps[threadId][idx] >
        ms) {
      haveFullSet = true;
      break;
    }
  ...
  const double time =
      static_cast<double>(lastestStamp - earliestStamp) / 1000.0;
