...
xmrig::CpuWorker<N>::CpuWorker(size_t index, const CpuLaunchData &data)
    : Worker(index, data.affinity, data.priority), m_algorithm(data.algorithm),
      m_assembly(data.assembly), m_hwAES(data.hwAES), m_av(data.av()),
      m_miner(data.miner), m_ctx() {
...
template <size_t N> bool xmrig::CpuWorker<N>::selfTest() {
  ...
  if (m_algorithm.family() == Algorithm::CN) {
    const bool rc = verify(Algorithm::CN_0, test_output_v0) &&
                    ...
                    verify(Algorithm::CN_BBC, test_output_bbc);
